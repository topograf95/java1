package ru.progwards.java1.SeaBattle.topograf95;
...
public class SeaBattleAlg {
    ...
    void algorithm1() {
        for (int y = 0; y < seaBattle.getSizeY(); y++) {
            for (int x = 0; x < seaBattle.getSizeX(); x++) {
                if (x == y || x + y == seaBattle.getSizeX() - 1) fireAndKill(x, y);
            }
        }
    }
    ...
    void stepFire(int offset) {
        for (int x = 0; x < seaBattle.getSizeY(); ++x) {
            for (int y = x + offset; y < seaBattle.getSizeX(); y += 4)
                fireAndKill(x, y);

            for (int y = x - offset; y >= 0; y -= 4)
                fireAndKill(x, y);
        }
    }
    void stepFire2(int offset) {
        for (int x = 0; x < seaBattle.getSizeX(); ++x) {
            for (int y = offset - x; y < seaBattle.getSizeY(); y += 4) {
                fireAndKill(x, y);
            }
        }
    }
    ...
    void algorithm3() {
        stepFire2(3);
        stepFire2(1);
 //       System.out.println("offset_0");
        stepFire2(0);
 //       System.out.println("offset_2");
        stepFire2(2);
    }
